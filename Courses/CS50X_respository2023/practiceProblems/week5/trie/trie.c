...
bool check(char *word)
{
    ...
    for (int i = 0, n = strlen(word); i < n; i++)
    {
        ...
        if (cursor->children[index] == NULL)
            return false;
        // End of word
        else if ((cursor->children[index] != NULL) && cursor->is_word == true)
            return true;
        // Letter found and not the end of the word
        else
            cursor = cursor->children[index]; // Move to that letter in the node, and follow path
    }
    return true;
}
